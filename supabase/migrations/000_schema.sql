-- =============================================================================
-- FlowCRM - Schema Base (CRM Puro)
-- =============================================================================
-- Para projetos NOVOS: Execute no SQL Editor do Supabase
-- Para RESETAR um projeto existente: Use reset.sql primeiro
-- Para adicionar Stripe: Execute 001_add_stripe.sql
-- =============================================================================
...
CREATE TABLE public.profiles (
    ...
    name TEXT,
    avatar TEXT,
    role TEXT DEFAULT 'user',                     -- Role dentro da empresa: 'admin', 'vendedor', 'user'
...
-- -----------------------------------------------------------------------------
-- 4. LIFECYCLE_STAGES (Estágios do funil - GLOBAL, igual pra todos)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 13. TAGS (Sistema de tags - por tenant)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 14. CUSTOM_FIELD_DEFINITIONS (Campos personalizados - por tenant)
-- -----------------------------------------------------------------------------
...
-- =============================================================================
-- 21. COMPANY_INVITES (Convites para novos usuários)
-- -----------------------------------------------------------------------------
CREATE TABLE public.company_invites (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    company_id UUID REFERENCES public.companies(id) ON DELETE CASCADE,
    email TEXT, -- Pode ser nulo para links genéricos
...
-- Pegar company_id do usuário atual (só se empresa ativa)
...
-- Estatísticas do Dashboard (filtrado por tenant)
...
BEGIN
    ...
    SELECT json_build_object(
        ...
        'pipeline_value', (SELECT COALESCE(SUM(value), 0) FROM public.deals WHERE company_id = user_company AND status NOT IN ('CLOSED_WON', 'CLOSED_LOST')),
        ...
        'won_deals', (SELECT COUNT(*) FROM public.deals WHERE company_id = user_company AND status = 'CLOSED_WON'),
        'won_value', (SELECT COALESCE(SUM(value), 0) FROM public.deals WHERE company_id = user_company AND status = 'CLOSED_WON'),
...
-- =============================================================================
-- OTIMIZADO: Usa JWT claim (auth.jwt()->>'company_id') em vez de get_user_company_id()
-- Isso elimina uma query extra por operação (20x mais rápido segundo Supabase docs)
-- =============================================================================

-- COMPANIES - usuário só vê sua própria empresa (e não deletada)
...
-- PROFILES - pode ler próprio perfil OU perfis da mesma empresa
CREATE POLICY "tenant_isolation_select" ON public.profiles
FOR SELECT TO authenticated
USING (
    id = (SELECT auth.uid())  -- Sempre pode ler próprio perfil
    OR company_id = (SELECT company_id FROM profiles WHERE id = (SELECT auth.uid()))  -- Ou da mesma empresa
);
...
FOR SELECT TO authenticated
USING (company_id = (auth.jwt()->>'company_id')::uuid AND (auth.jwt()->>'user_role') = 'admin');

CREATE POLICY "Admins can create invites" ON public.company_invites
FOR INSERT TO authenticated
WITH CHECK (company_id = (auth.jwt()->>'company_id')::uuid AND (auth.jwt()->>'user_role') = 'admin');
...
-- =============================================================================
-- AUTH HOOK: Custom Access Token (Injeta company_id no JWT)
-- =============================================================================
-- Isso permite usar (auth.jwt()->>'company_id')::uuid nas policies
-- Em vez de chamar get_user_company_id() que faz query extra
-- =============================================================================
...
DECLARE
  ...
  user_role text;
BEGIN
  -- Busca company_id e role do profile
  SELECT p.company_id, p.role 
  INTO user_company, user_role
  FROM public.profiles p
  JOIN public.companies c ON c.id = p.company_id
  ...
  -- Injeta company_id no JWT
  IF user_company IS NOT NULL THEN
    claims := jsonb_set(claims, '{company_id}', to_jsonb(user_company::text));
  END IF;
  
  -- Injeta user_role no JWT (admin, vendedor, user)
...
-- Grants para o Auth Hook (IMPORTANTE: só supabase_auth_admin pode executar)
...
--
-- AUTH HOOK:
-- custom_access_token_hook - Injeta company_id e user_role no JWT
-- ⚠️  IMPORTANTE: Ativar no Supabase Dashboard > Authentication > Hooks
--
-- MIGRATIONS DISPONÍVEIS:
-- • 001_add_stripe.sql - Integração com Stripe (quando precisar)
--
-- FILOSOFIA: "Simplicity is the ultimate sophistication"
-- Só existe o que você usa. YAGNI.
-- =============================================================================
